...
!https://travis-ci.org/cdamus/emf.compare.svg?branch=uml-facade!:https://travis-ci.org/cdamus/emf.compare

h2. Installation

* The latest version of EMF Compare can be installed from http://download.eclipse.org/modeling/emf/compare/updates/releases/
...
* Eclipse version 4.4 (Luna) to Eclipse version 4.7 (Oxygen)
...
|_. Feature                          |_. Eclipse 4.7 (Oxygen) |_. Eclipse 4.6 (Neon) |_. Eclipse 4.5 (Mars) |_. Eclipse 4.4 (Luna) |
| EMF Compare Core                   | X                      | X                    | X                    | X                      |
| EMF Compare RCP UI                 | X                      | X                    | X                    | X                      |
| EMF Compare IDE UI                 | X                      | X                    | X                    | X                      |
| EMF Compare UML integration        | X                      | X                    | X                    | X                      |
| EMF Compare GMF integration        | X                      | X                    | X                    | X                      |
| EMF Compare EcoreTools integration | X                      | X                    | X                    | X                      |
| EMF Compare Papyrus integration    | X                      | X                    | X                    | X                      |
| EMF Compare Façade Models          | X                      |                      |                      |                        |
...
@mvn clean package -Poxygen@

The above example will build the core EMF Compare binaries against an Oxygen platform. Select the appropriate profile according to the platform you wish to install EMF Compare on, one of [oxygen, neon, mars, luna].

You can also check the tests using

@mvn verify -Poxygen --fail-at-end -Dmaven.test.failure.ignore=true@

For additional, non-core components, activate additional profiles as required:

* for the façade-based compare/merge extensions: @mvn clean package -Poxygen -Pfacade-modules@
* for integrations with Eclipse modeling platforms such Papyrus and Sirius: @mvn clean package -Poxygen-extras -Pextra-modules@

Note the variant of the target platform profile for inclusion of additional dependencies not required by the EMF Compare core and façade components.
