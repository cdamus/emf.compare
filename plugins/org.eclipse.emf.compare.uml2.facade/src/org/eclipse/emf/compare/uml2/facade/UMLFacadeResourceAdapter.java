...
import org.eclipse.emf.compare.facade.ReflectiveDispatch;
...
public class UMLFacadeResourceAdapter extends AdapterImpl {
	...
	/** The logger. */
	private static final Logger LOGGER = Logger.getLogger(UMLFacadeResourceAdapter.class);
	...
	public static <T extends UMLFacadeResourceAdapter> T getInstance(Resource resource, Class<T> type) {
		...
		if (result == null) {
			try {
				...
				LOGGER.error("Failed to create resource adapter", e); //$NON-NLS-1$
			}
		}
