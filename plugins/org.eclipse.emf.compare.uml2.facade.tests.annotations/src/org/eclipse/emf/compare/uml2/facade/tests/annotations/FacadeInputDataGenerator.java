...
/**
 * This is the {@code FacadeInputDataGenerator} type. Enjoy.
 *
 * @author Christian W. Damus
 */
@SupportedSourceVersion(SourceVersion.RELEASE_6)
@SupportedAnnotationTypes("org.eclipse.emf.compare.uml2.facade.tests.data.FacadeInput")
public class FacadeInputDataGenerator extends AbstractProcessor {

	/**
	 * Initializes me.
	 */
	public FacadeInputDataGenerator() {
		super();
	}
	...
	public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
		...
		TypeElement facadeInputType = annotations.iterator().next();
		Types types = processingEnv.getTypeUtils();

		for (TypeElement type : ElementFilter.typesIn(roundEnv.getElementsAnnotatedWith(facadeInputType))) {

			AnnotationMirror facadeInput = type.getAnnotationMirrors().stream()
					.filter(m -> types.asElement(m.getAnnotationType()).equals(facadeInputType)).findFirst()
					.get();
			...
			try {
				...
				try (PrintWriter output = new PrintWriter(file.openWriter())) {
					...
					if (!package_.getQualifiedName()
							.contentEquals("org.eclipse.emf.compare.uml2.facade.tests.data")) { //$NON-NLS-1$
						output.println(
								"import org.eclipse.emf.compare.uml2.facade.tests.data.AbstractFacadeInputData;"); //$NON-NLS-1$
					}
					output.println();
					output.printf("@Generated(date=\"%s\", value=\"%s\")%n", //$NON-NLS-1$
							DateTimeFormatter.ISO_LOCAL_DATE_TIME.format(LocalDateTime.now()),
							getClass().getName());
					output.printf("public abstract class %s extends AbstractFacadeInputData {%n%n", genName); //$NON-NLS-1$
	...
	protected void generateAccessor(PrintWriter output, String scenarioName, String side) {
		...
		output.printf("\t\treturn loadFacadeFromClassLoader(\"%s/%s.uml\"); //$NON-NLS-1$%n", scenarioName, //$NON-NLS-1$
				side);
		output.println("\t}"); //$NON-NLS-1$
		output.println();
